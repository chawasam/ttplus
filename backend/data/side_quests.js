...
const SIDE_QUESTS = [
  ...
  // ─── Sythara — ความลับของ Sealer (affection 50+) ─────────────────────────────
  {
    id:       'sythara_secret_1',
    name:     'สิ่งที่ Sythara ซ่อนไว้',
    category: 'personal',
    zone:     'darkroot_hollow',
    giverNpc: 'sythara',
    prereqs:  [],
    minLevel: 15,
    minAffection: { npcId: 'sythara', amount: 50 },
    autoAccept: false,
    desc: 'Sythara ขอให้คุณตามหาบันทึกส่วนตัวที่เธอซ่อนไว้ในพิธีผนึก บันทึกที่เธอไม่เคยกล้าอ่านอีกครั้ง',
    steps: [
      { id: 'talk_sythara_truth', type: 'talk', target: 'sythara', count: 1,
        hint: 'พูดคุยกับ Sythara เพื่อฟังเรื่องราวก่อนพิธีผนึก' },
      { id: 'explore_darkroot_ritual', type: 'explore', target: 'darkroot_hollow', count: 3,
        hint: 'สำรวจ Darkroot Hollow 3 ครั้งเพื่อหาสถานที่พิธีโบราณ' },
      { id: 'find_sythara_journal', type: 'explore', target: 'shadowfell_depths', count: 2,
        hint: 'ค้นหาบันทึกใน Shadowfell Depths' },
    ],
    rewards: { xp: 800, gold: 500, items: ['ancient_scroll', 'mp_potion_medium'] },
    completionText: 'Sythara อ่านบันทึกนั้นด้วยมือที่สั่นเบาๆ "ฉันรู้จักพวกเขาก่อนที่พวกเขาจะยอมสละตัว..." น้ำตาไหลออกมา "ขอบคุณที่ช่วยฉันเผชิญหน้ากับมัน"',
  },
  {
    id:       'sythara_secret_2',
    name:     'สิ่งที่ Sythara ทำไม่ได้ห้าม',
    category: 'personal',
    zone:     'shadowfell_depths',
    giverNpc: 'sythara',
    prereqs:  ['sythara_secret_1'],
    minLevel: 22,
    minAffection: { npcId: 'sythara', amount: 70 },
    autoAccept: false,
    desc: 'Sythara รู้ว่าตราผนึกจะพังเพราะเธอมองเห็นจุดอ่อนมาตลอด เธอต้องการพูดความจริงกับผู้ที่ถูกผนึก',
    steps: [
      { id: 'enter_shadowfell_sythara', type: 'travel', target: 'shadowfell_depths', count: 1,
        hint: 'เดินทางไปยัง Shadowfell Depths' },
      { id: 'kill_echo_guard', type: 'kill', target: 'shadow_wraith', count: 5,
        hint: 'กำจัด Shadow Wraith 5 ตัว เพื่อปกป้อง Sythara' },
      { id: 'talk_echo_sythara', type: 'talk', target: 'sythara', count: 1,
        hint: 'ให้ Sythara พูดคุยกับ The Echo' },
    ],
    rewards: { xp: 1200, gold: 800, items: ['void_crystal', 'guardian_stone'] },
    completionText: 'The Echo ฟังทุกอย่างโดยไม่พูดอะไร แล้วพยักหน้า "ฉันรู้แล้วว่าใครส่งเจ้ามา Sythara เก่า... ดูแลเธอด้วยนะ"',
  },

  // ─── Corvin — สินค้าที่หายไป (affection 40+) ──────────────────────────────────
  {
    id:       'corvin_mystery_1',
    name:     'สินค้าที่ไม่เคยส่งถึง',
    category: 'personal',
    zone:     'voidspire_ruins',
    giverNpc: 'corvin',
    prereqs:  [],
    minLevel: 18,
    minAffection: { npcId: 'corvin', amount: 40 },
    autoAccept: false,
    desc: 'Corvin ขอให้คุณตามหา "พัสดุ" ที่หายไปใน Sunken Crypts สินค้าที่เขาไม่เคยบอกว่าเป็นอะไร',
    steps: [
      { id: 'talk_corvin_package', type: 'talk', target: 'corvin', count: 1,
        hint: 'พูดคุยกับ Corvin เพื่อรับรายละเอียด' },
      { id: 'explore_crypts_corvin', type: 'explore', target: 'sunken_crypts', count: 4,
        hint: 'ค้นหาสินค้าใน Sunken Crypts 4 ครั้ง' },
      { id: 'kill_crypt_guards', type: 'kill', target: 'void_priest', count: 3,
        hint: 'กำจัด Void Priest ที่เฝ้ายาม 3 ตัว' },
    ],
    rewards: { xp: 900, gold: 700, items: ['memory_fragment', 'ancient_scroll', 'health_potion_large'] },
    completionText: 'Corvin รับพัสดุโดยไม่เปิดให้ดู "ขอบคุณมาก. ไม่ใช่ของที่คุณคิด — เป็นหลักฐาน. หลักฐานที่ทำให้คนบางคนอยากเห็นฉันตาย" เขายิ้มลึกลับ "ยังไม่ถึงเวลาที่คุณจะรู้ทุกอย่าง"',
  },
  {
    id:       'corvin_mystery_2',
    name:     'R.H.',
    ...
    zone:     'voidspire_ruins',
    giverNpc: 'corvin',
    prereqs:  ['corvin_mystery_1'],
    minLevel: 28,
    minAffection: { npcId: 'corvin', amount: 70 },
    autoAccept: false,
    desc: 'Corvin พร้อมที่จะบอกความจริงแล้ว ตัวตนจริงของเขา และว่าทำไมเขาถึงปรากฏตัวทุกที่',
    steps: [
      { id: 'travel_voidspire_corvin', type: 'travel', target: 'voidspire_ruins', count: 1,
        hint: 'พบ Corvin ที่ Voidspire Ruins' },
      { id: 'explore_void_mirror', type: 'explore', target: 'voidspire_ruins', count: 5,
        hint: 'ค้นหาห้องลับที่มีกระจก Void (สำรวจ 5 ครั้ง)' },
      { id: 'talk_corvin_truth', type: 'talk', target: 'corvin', count: 1,
        hint: 'ให้ Corvin อธิบายสิ่งที่เห็นในกระจก' },
    ],
    rewards: { xp: 1500, gold: 1200, items: ['void_crystal', 'chaos_shard', 'memory_fragment'] },
    completionText: 'กระจก Void เผยภาพ: Corvin ยืนอยู่กับเงาที่ไม่มีเงา "ใช่... นั่นคือฉัน. ชื่อจริงของฉันคือ Rael Hask — R.H. นักเวทย์คนที่ 6 ที่ควรอยู่ในพิธีผนึก แต่หนีออกมา. ฉันใช้ชีวิตทั้งชีวิตทดแทนสิ่งนั้น"',
  },
];
