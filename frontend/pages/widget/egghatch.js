...
// กด R ในเบราว์เซอร์เพื่อ reset ไข่ใหม่
...
// SVG crack paths — viewBox 0 0 120 160 (ไข่ cx=60 cy=88 rx=46 ry=58)
const CRACK_STAGES = [
  [],
  // stage 1 — รอยร้าวเล็กที่ยอด
  ['M 60,32 L 56,46 L 63,40 Z'],
  // stage 2 — แตกลงมา + ด้านข้าง
  ['M 60,30 L 54,52 L 64,44 L 58,62', 'M 46,68 L 41,80'],
  // stage 3 — กระจายมากขึ้น
...
export default function EggHatchWidget() {
  ...
  // สีไข่ตาม progress
  const eggLightness = 88 - pct * 15;
  const eggSat       = 55 + pct * 20;
  const eggHue       = 38 - pct * 15;
  const eggFill      = phase === 'hatching' ? '#fffde7' : `hsl(${eggHue},${eggSat}%,${eggLightness}%)`;
  ...
  const wobbleAnim =
    ...
    crackStage >= 2      ? 'eggWobble 2.2s ease-in-out infinite'  :
    'none';
    ...
    <div style={{
      ...
      <style>{`
        ...
        @keyframes eggHatch {
          ...
          30%  { transform: scale(1.2) rotate(-6deg); filter: brightness(1.5); }
          55%  { transform: scale(1.5) rotate(6deg);  opacity: 0.7; filter: brightness(3); }
          100% { transform: scale(2.2) rotate(0deg);  opacity: 0; }
        }
        @keyframes creatureIn {
          0%   { transform: scale(0.2) translateY(30px); opacity: 0; filter: brightness(3); }
          55%  { transform: scale(1.15) translateY(-8px); opacity: 1; filter: brightness(1.5); }
        ...
        {phase === 'hatched' ? (
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{
              ...
              filter: 'drop-shadow(0 0 28px rgba(255,200,50,0.95)) drop-shadow(0 0 8px rgba(255,255,255,0.5))',
              animation: 'creatureIn 0.9s ease both, creatureFloat 3s 1s ease-in-out infinite',
              display: 'block',
            }}>{creature}</div>
          </div>
        ) : (
          ...
          <div style={{ animation: wobbleAnim }}>
            <svg viewBox="0 0 120 160" width="160" height="213" xmlns="http://www.w3.org/2000/svg">
              ...
              {/* Inner light glow (ยิ่งใกล้ฟัก ยิ่งสว่าง) */}
              {pct > 0.15 && (
                <ellipse cx="60" cy="95" rx="32" ry="42"
                  fill={innerColor}
                  style={{ animation: 'innerPulse 1.4s ease-in-out infinite' }}
                />
              )}

              {/* Egg body */}
              <ellipse cx="60" cy="88" rx="46" ry="58"
                fill={eggFill}
                stroke="rgba(160,120,60,0.5)"
                strokeWidth="1.5"
              />
              ...
              {/* Specular highlight */}
              <ellipse cx="46" cy="60" rx="10" ry="14"
        ...
        <div style={{ width: '78%', maxWidth: '210px' }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              animation: crackStage >= 3 ? 'barGlow 1.2s ease-in-out infinite' : 'none',
            }} />
          </div>
          {lastSender && (
            <div style={{ textAlign: 'center', marginTop: '6px', fontSize: '11px', color: 'rgba(255,255,255,0.35)' }}>
              ✨ {lastSender}
            </div>
          )}
        </div>
